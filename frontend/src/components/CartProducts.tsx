...
import { decrementQuantity, incrementQuantity } from "@/lib/cart/cartSlice";
        ...
        {items.length === 0 ? (
          ...
          <ul>
            {items.map((item) => (
              <li
                key={item._id}
                className="bg-white rounded-[12px] w-full flex justify-between items-center pl-1 py-3 pr-14 not-first:mt-6"
                style={{
